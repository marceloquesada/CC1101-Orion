void CCinit(int TxPower = 10){
  if (Orion_cc1101.getCC1101()){      // Check the CC1101 Spi connection.
    ...
    displayLine("TX connection error");
  }
  ...
  Orion_cc1101.setDeviation(2);   // Set the Frequency deviation in kHz. Value from 1.58 to 380.85. Default is 47.60 kHz.
  ...
  Orion_cc1101.setPA(TxPower);             // Set TxPower. The following settings are possible depending on the frequency band.  (-30  -20  -15  -10  -6    0    5    7    10   11   12) Default is max!
...
unsigned char* generatePacket(bool comm_status, int upkeep, float pres, float temp, float alt, char payload[]){
  ...
  packet[p_index] = comm_status ? 1 : 0;
  p_index++;

  // Segundo byte representa a manutenção
  packet[p_index] = (upkeep >> 8) & 0xFF; // Byte mais significativo
  p_index++;
  packet[p_index] = upkeep & 0xFF; // Byte menos significativo
  p_index++;

  // Próximos quatro bytes representam a pressão
  unsigned char *p_pres = (unsigned char *)&pres;
  for (int i = 0; i < sizeof(float); i++) {
    packet[p_index + i] = p_pres[i];
  }
  p_index+=sizeof(float);

  // Próximos quatro bytes representam a temperatura
  unsigned char *p_temp = (unsigned char *)&temp;
  for (int i = 0; i < sizeof(float); i++) {
    packet[p_index + i] = p_temp[i];
  }
  p_index+=sizeof(float);

  // Próximos quatro bytes representam a altitude
