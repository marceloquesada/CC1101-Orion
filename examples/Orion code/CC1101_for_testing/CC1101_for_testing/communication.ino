...
void sendPacket()
{
  Orion_cc1101.SendPktData(tx_msg, sizeof(tx_msg));
  displayLine("TX PKT OK");
}

void fillPacket(int tx_type)
{
  byte tx_byte;
...
unsigned char* generatePacket(bool comm_status, int upkeep, float pres, float temp, float alt, char payload[]){
  int packet_length = 63;
  byte packet[packet_length];

  byte b_comm_status = (byte *)&comm_status;
  byte b_upkeep = (byte *)&upkeep;

  packet[0] = b_comm_status
