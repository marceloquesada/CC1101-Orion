...
void setup(){
    ...
    Orion_cc1101.setMHZ(437);        // Here you can set your basic frequency. The lib calculates the frequency automatically (default = 433.92).The cc1101 can: 300-348 MHZ, 387-464MHZ and 779-928MHZ. Read More info from datasheet.
    Orion_cc1101.setDeviation(1.58);   // Set the Frequency deviation in kHz. Value from 1.58 to 380.85. Default is 47.60 kHz.
    Orion_cc1101.setChannel(0);         // Set the Channelnumber from 0 to 255. Default is cahnnel 0.
    ...
    Orion_cc1101.setRxBW(812.50);       // Set the Receive Bandwidth in kHz. Value from 58.03 to 812.50. Default is 812.50 kHz.
    Orion_cc1101.setDRate(0.02);       // Set the Data Rate in kBaud. Value from 0.02 to 1621.83. Default is 99.97 kBaud!
    ...
    Orion_cc1101.setSyncMode(0);        // Combined sync-word qualifier mode. 0 = No preamble/sync. 1 = 16 sync word bits detected. 2 = 16/16 sync word bits detected. 3 = 30/32 sync word bits detected. 4 = No preamble/sync, carrier-sense above threshold. 5 = 15/16 + carrier-sense above threshold. 6 = 16/16 + carrier-sense above threshold. 7 = 30/32 + carrier-sense above threshold.
    //Orion_cc1101.setSyncWord(211, 145); // Set sync word. Must be the same for the transmitter and receiver. (Syncword high, Syncword low)
    ...
    Orion_cc1101.setLengthConfig(2);    // 0 = Fixed packet length mode. 1 = Variable packet length mode. 2 = Infinite packet length mode. 3 = Reserved
    ...
    //Orion_cc1101.setAGC(0x90, 0x49, 0x43); // Sets the address values for the RX AGC, check the Smart RF Studio software to configure these values
...
void loop(){
  ...
  if (Orion_cc1101.CheckRxFifo(5000)){

  //Rssi Level in dBm
  Serial.print("Rssi: ");
  Serial.println(Orion_cc1101.getRssi());

  //Link Quality Indicator
  Serial.print("LQI: ");
  Serial.println(Orion_cc1101.getLqi());

  //Get received Data and calculate length
  Orion_cc1101.ReceiveData(buffer, read_size);
  buffer[read_size] = '\0';
